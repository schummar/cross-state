...
describe('propAccess', () => {
  describe.each([
    ...
    test('get', async () => {
      const calculatedValue = get(obj, path as any);
    ...
    test('set', async () => {
      ...
      if (newObj instanceof Error) {
        expect(() => set(obj, path as any, newValue)).toThrow('Cannot set x.y.z because x.y is undefined');
      } else {
        const updated = set(obj, path as any, newValue);
