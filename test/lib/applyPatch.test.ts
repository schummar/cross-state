...
describe('applyPatch', () => {
  ...
  test('map', () => {
    ...
    expect(applyPatches(new Map([['x', 1]]), { op: 'replace', path: ['x'], value: 2 })).toEqual(
      new Map([['x', 2]]),
    );
  });
  ...
  test('set', () => {
    ...
    expect(applyPatches(new Set([1]), { op: 'replace', path: [0], value: 2 })).toEqual(
      new Set([2]),
    );
  });
  ...
  test('nested', () => {
    expect(
      applyPatches({ x: 1, y: { z: 2 } }, { op: 'replace', path: ['y', 'z'], value: 3 }),
    ).toEqual({
      x: 1,
      y: { z: 3 },
    });
